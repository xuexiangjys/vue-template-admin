  ...
  <div class="fillcontain">
    <div class="table_container">
      ...
      <el-dialog title="修改用户信息" :visible.sync="dialogFormVisible">
        <el-form :model="selectData" :rules="formrules" ref="editForm" label-width="100px">
          <el-form-item label="用户名" prop="loginName">
            <el-input v-model="selectData.loginName" placeholder="请输入用户名（登录使用）"></el-input>
          </el-form-item>
          <el-form-item label="密码" prop="password">
            <el-input v-model="selectData.password" placeholder="请输入密码（登录使用）"></el-input>
          </el-form-item>
          <el-form-item label="别名" prop="nick">
            <el-input v-model="selectData.nick" placeholder="请输入别名"></el-input>
          </el-form-item>
          <el-form-item label="权限" prop="authority">
            <el-select v-model="selectData.authority" placeholder="请选择用户类型">
              <el-option label="管理员" value="admin"></el-option>
              <el-option label="普通用户" value="editor"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="手机号" prop="phone">
            <el-input v-model="selectData.phone" placeholder="请输入手机号" oninput="value=value.replace(/[^\d]/g,'')">
            </el-input>
          </el-form-item>
          <el-form-item label="注册时间" prop="registerTime">
            <el-date-picker v-model="selectData.registerTime" align="right" type="datetime" placeholder="选择注册时间"
              value-format="timestamp">
            </el-date-picker>
          </el-form-item>
          <el-form-item label="地址" prop="address">
            <el-input v-model="selectData.address" placeholder="请输入用户注册地址"></el-input>
          </el-form-item>
        </el-form>
...
<script>
  ...
  export default {
    ...
    methods: {
      fetchData() {
        getPagingUsers(this.listQuery).then(response => {
          const pageData = response.data;
          if (pageData) {
            this.tableData = pageData.array;
      ...
      handleEdit(index, row) {
        this.selectData = Object.assign({}, row) // copy obj
        ...
        this.$confirm("用户删除后将无法恢复, 是否继续?", "提示", {
          ...
          .then(() => {
            deleteUser(row).then(response => {
              if (response.data) {
                deleteTableItem(this.tableData, row, 'userId');
                this.$message({
                  type: "success",
                  message: "删除成功!"
                });
      ...
      updateInfo() {
        this.$refs['editForm'].validate(valid => {
          if (valid) {
            updateUser(this.selectData).then(response => {
              if (response.data) {
                this.dialogFormVisible = false;
                updateTableItem(this.tableData, this.selectData, 'userId');
                this.$message({
                  type: "success",
                  message: "编辑成功!"
                });
